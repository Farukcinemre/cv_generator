...
class PDF(FPDF):
    def __init__(self, theme_color=(0, 0, 0)):
        ...
        # Font yolları - Küçük harf kullanımı Linux/Docker uyumu için kritiktir
        f_reg = "arial.ttf"
        f_bold = "arialbd.ttf"
        f_ital = "ariali.ttf"

        # Varsayılan font (Eğer dosyalar bulunamazsa Helvetica'ya döner)
        self.font_family_to_use = "helvetica" 

        if os.path.exists(f_reg):
            try:
                # uni=True parametresi fpdf2'de Unicode desteği sağlar
                ...
                if os.path.exists(f_bold):
                    ...
                    self.add_font("ArialTR", "B", f_reg) # Bold yoksa Regular kullan
                
                if os.path.exists(f_ital):
                    self.add_font("ArialTR", "I", f_ital)
                else:
                    self.add_font("ArialTR", "I", f_reg) # İtalik yoksa Regular kullan
            except Exception:
                self.font_family_to_use = "helvetica"
...
if st.button("✨ CV'Yİ PDF OLARAK OLUŞTUR"):
    if not isim:
        ...
        try:
            ...
            if deneyimler:
                ...
                for dn in deneyimler:
                    if dn.get('u'):
                        ...
                        pdf.cell(0, 7, dn.get('t', ''), ln=True, align="R")
                        pdf.set_font(pdf.font_family_to_use, "", 10)
                        for m in dn.get('d', []):
            ...
            # --- HATA ÇÖZÜMÜ: bytearray to bytes conversion ---
            pdf_output = pdf.output()
            # Eğer fpdf2 ise direkt bytes döner, değilse çevirmek gerekebilir
            pdf_bytes = bytes(pdf_output) if isinstance(pdf_output, (bytearray, str)) else pdf_output

            st.sidebar.download_button(
                label="✅ PDF'İ ŞİMDİ İNDİR", 
                data=pdf_bytes, 
                file_name=f"{isim.replace(' ', '_')}_CV.pdf", 
